        ...
        {!isStill && (
          <div className="catalog-card-meta">
            {movie.vote_average > 0 && (
              <span className="catalog-card-meta-rating">
                <FaStar size={11} /> {movie.vote_average.toFixed(1)}
              </span>
            )}
            {year && <span className="catalog-card-meta-item">{year}</span>}
            <span className="catalog-card-meta-item">{mediaType === 'tv' ? 'TV Show' : 'Movie'}</span>
          </div>
        )}
      </div>
    </div>
